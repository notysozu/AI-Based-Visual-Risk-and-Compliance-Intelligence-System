...
async def jarvis_voice_assistant(req: Dict[str, Any]):
    """
    JARVIS Autonomous Cockpit Copilot endpoint.
    Routes queries to the 100-Agent Swarm Orchestrator with persistent MongoDB memory.
    """
    from ai_engine.agents.orchestrator import JarvisAgentSwarmOrchestrator
    user_id = str(req.get("user_id") or "1")
    prompt = (req.get("prompt") or "").strip()
    subject = (req.get("subject") or "General Study").strip()
    history = req.get("history") or []
    cockpit_context = req.get("cockpit_context") or {}

    result = await JarvisAgentSwarmOrchestrator.process_turn(
        user_id=user_id,
        prompt=prompt,
        subject=subject,
        history=history,
...
@router.get("/jarvis/memories/{user_id}")
async def list_jarvis_memories(user_id: str, category: Optional[str] = None):
    """Retrieve all persistent long-term memory facts stored for the user by JARVIS."""
    memories = await crud.get_jarvis_memories(user_id, category=category, limit=50)
    return [
        {
            "id": str(m.id),
            "user_id": m.user_id,
...
@router.post("/jarvis/memories/{user_id}")
async def create_jarvis_memory(user_id: str, req: Dict[str, Any]):
    """Save or update a persistent memory fact in JARVIS long-term memory."""
    memory_key = req.get("memory_key") or f"fact_{int(datetime.utcnow().timestamp())}"
    content = req.get("content") or ""
    category = req.get("category") or "general"
    importance = float(req.get("importance") or 1.0)

    if not content:
        raise HTTPException(status_code=400, detail="Content cannot be empty")

    mem = await crud.save_jarvis_memory(
        user_id=user_id,
...
@router.delete("/jarvis/memories/{user_id}/{memory_id}")
async def delete_jarvis_memory_item(user_id: str, memory_id: str):
    """Delete a specific JARVIS memory item."""
    success = await crud.delete_jarvis_memory(user_id, memory_id)
    if not success:
...
async def list_jarvis_swarm_agents():
    """List all 100 specialized sub-agents in the JARVIS Swarm."""
    from ai_engine.agents.swarm_registry import list_all_agents
