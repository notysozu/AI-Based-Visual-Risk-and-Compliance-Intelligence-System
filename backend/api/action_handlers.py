...
async def execute_action_payload(user: models.UserDoc, action_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if action_type in ["add_task", "add_multiple_tasks"]:
        ...
        if cost > 0:
            rec = models.FinancialRecordDoc(
                user_id=u_id_str,
                amount=cost,
                category="Major Purchase",
                description=f"Purchase of {payload.get('item_name', 'Item')}",
        ...
        study_rec = models.StudyRecordDoc(
            user_id=u_id_str,
            subject=subject,
        ...
        execution_result = {
            "habit_record_id": str(habit_rec.id),
            "habit_name": h_name,
            "duration_minutes": dur_mins,
            "impact_score": impact_score
        }
