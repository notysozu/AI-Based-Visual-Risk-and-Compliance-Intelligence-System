...
## 2. Multi-Agent Orchestration & Action Execution Engine

Unlike passive chatbots, the Visual Risk Copilot incorporates a **Multi-Agent Orchestrator** (`ai_engine/agents/router.py`) that intelligently routes user inquiries to dedicated domain sub-agents (GoalAgent, FinanceAgent, SettingsAgent, PlannerAgent, StudyAgent, HabitAgent) or falls back to scenario modeling.

Mutations are dispatched with `auto_execute` status to persist directly to MongoDB and synchronize client state without requiring manual approval clicks:

```mermaid
flowchart LR
  Copilot["Visual Risk Copilot Chat (/chat)"] --> Router{"Multi-Agent Router (Priority Order)"}

  subgraph DirectMutationAgents["Direct Mutation Sub-Agents (auto_execute)"]
    Router -->|Goal Commands| A_Goal["GoalAgent (update_goal)<br/>• Changes goal name & target<br/>• Auto-calibrates item costs (MacBook, etc.)"]
    Router -->|Income/Expenses/Net Worth| A_Fin["FinanceAgent (update_user_fields)<br/>• Updates cashflow ledger<br/>• Recalculates surplus & savings rate"]
    Router -->|Sleep/Study/Retire/Role| A_Set["SettingsAgent (update_user_fields)<br/>• Modifies lifestyle targets & theme<br/>• Persists to MongoDB UserDoc"]
    Router -->|Single Task Addition| A_Plan["PlannerAgent (add_task)<br/>• Extracts title, time & duration<br/>• Direct insert into UserDoc.tasks_json"]
    Router -->|Coursework Logging| A_Study["StudyAgent (log_study)<br/>• Inserts StudyRecordDoc & focus score<br/>• Updates weekly academic pace"]
    Router -->|Biometric Logging| A_Habit["HabitAgent (log_habits_batch)<br/>• Records sleep, exercise, screen, mood<br/>• Appends to HabitRecordDoc"]
  end

  subgraph AnalyticalFallbacks["Exploratory & Analytical Flows (proposed)"]
    Router -->|Major Purchase Friction| A_Purch["purchase_impact<br/>• Evaluates milestone friction & 5-Yr CAGR"]
    Router -->|Scenario Sandbox| A_Sim["simulate_what_if<br/>• Sandboxes lifestyle trade-offs into Scenario B"]
    Router -->|Wealth Projection| A_Wealth["wealth_forecast<br/>• 500-run Monte Carlo probability"]
    Router -->|Daily Schedule Blueprint| A_Multi["add_multiple_tasks<br/>• Full circadian day schedule synthesis"]
  end
```
